...
const splitMe = function(string){
  let nArray = string.split(' ')
 return nArray
}

console.log(splitMe('ciao come va oggi'))
...
const onlyLetters = function(string){
  for(let i = 0; i < 10; i++){
 //   string=string.replace(${i},"")            //Procesing 
  }
}
...
const whatDayIsIt = function(){
  let week = ['domenica', 'Lunedi', 'Martedi', 'Mercoli', 'Giovedi', 'Venerdi', 'Sabato']
  const date = new Date()
  const dayweek = date.getDay()
  return week[dayweek]
}
const es7Today = whatDayIsIt()
console.log('oggi è', es7Today)
...
const isTodayMyBirthday = function (){
  ...
  if (isTodayMyBirthday()) {
    ...
    console.log('nope')                            //mi sono bloccato al return ..
  }
  return (
    now.getMonth() === month && 
    now.getDate() === day
...
const onlyInLastMillennium = function(){
  let millennio = movies.filter((movie) => {
...
//millennio = onlyInLastMillennium(movies)
...
const sumAllTheYears = function(){
  let sum = movies.reduce((acc, currentValue) => {
